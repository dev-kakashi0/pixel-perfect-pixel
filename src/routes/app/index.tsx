...
import { CheckCircle2, Clock, Home, Users } from "lucide-react";
...
export const Route = createFileRoute("/app/")({
  ...
  head: () => ({
    meta: [
      { title: "Tableau de bord — Foyers ONG Togo" },
      { name: "description", content: "Vue d'ensemble des maisons, des résidents et de votre profil." },
      { property: "og:title", content: "Tableau de bord — Foyers ONG Togo" },
    ...
    <div className="space-y-6">
      ...
      {profile?.statut === "en_attente" && (
        <div className="surface-card border-l-4 border-l-primary p-4">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            L'administrateur doit valider votre compte et vous assigner à une maison. Vous pouvez
            déjà compléter votre profil.
          </p>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {(isAdmin || isResponsable) && (
          <>
            <StatCard icon={Users} label="Résidents visibles" value={profiles.length} />
            <StatCard icon={CheckCircle2} label="Comptes validés" value={valides} />
            <StatCard icon={Clock} label="En attente" value={enAttente} />
          </>
        )}
        <StatCard icon={Home} label="Maisons de l'ONG" value={houses.length} />
      </div>
